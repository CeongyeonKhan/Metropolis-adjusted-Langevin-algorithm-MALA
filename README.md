...
本工程依据《能量梯度误差约束自适应混合 MALA 实验方案》实现，默认完成解析合成实验 P0—P7，并使用 PyTorch 进行向量化采样。正式采样中的接受能量固定为解析目标能量 \(E^*\)；冻结梯度集成只用于构造候选点。该边界保证Metropolis–Hastings, (MH校正消除的是给定目标下的离散化偏差与提议梯度误差，而不是把学习目标误差误解释为采样误差。
